...
def get_apprise_type(status):
    return STATUS_TYPE_MAPPING.get(status.lower(), status)

# Apprise: supported fields are body, title, notification_type, tag and format
...
def get_apprise_data(request):
    ...
    # copy commonLabels 'tag', 'format', 'notification_type' and 'title' to apprise_data when specified
    ...
    apprise_data['notification_type'] = get_apprise_type(apprise_data['notification_type'])
